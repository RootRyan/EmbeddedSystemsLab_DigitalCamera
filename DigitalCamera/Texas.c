...
void ADC1_Init(void){ volatile unsigned long delay;
  ...
  GPIO_PORTD_DIR_R &= ~0x04;      // 3) make PD3 input
  GPIO_PORTD_AFSEL_R |= 0x04;     // 4) enable alternate function on PD3
  GPIO_PORTD_DEN_R &= ~0x04;      // 5) disable digital I/O on PD3
  GPIO_PORTD_AMSEL_R |= 0x04;     // 6) enable analog functionality on PD3
