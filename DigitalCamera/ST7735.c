...
// ST7735.c
// Ryan Root
// Initial Creation Date: 8/31/21
// Working ST7735_sDecOut2, ST7735_uBinOut6, ST7735_XYplotInit, ST7735_XYplot functions for the LCD Display
// Lab 01
// TA: Hassan Iqbal
// Last Modified: 9/7/21
// Hardware Configuration: Specified in Lab1.c starter code (And predefined by the unified port for the LCD and on board buttons)
...
//Defining local variables for XY Plot methods
static int32_t plotXMin, plotXMax, plotYMin, plotYMax;
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[], uint16_t color) {
  for(int i=0;i<num;i++){
		//Checks if in min/max bounds and draws a a pixel at the x/y coordinate if it does
		if (bufX[i] > plotXMin && bufX[i] < plotXMax && bufY[i] > plotYMin && bufY[i] < plotYMax){ 
			int xCoordinate = (127*(bufX[i]-plotXMin)/(plotXMax-plotXMin)); 
			int yCoordinate = (32 + (127*(plotYMax - bufY[i])/(plotYMax-plotYMin))); 
			ST7735_DrawPixel(xCoordinate, yCoordinate, color);
		}
  }
}
