...
// I2C calls).
void OV7670_write_list(OV7670_command *cmd) {
...
OV7670_status OV7670_begin(OV7670_colorspace colorspace, OV7670_size size, float fps) {
  ...
  //Make sure ports are initialized
  ...
  // tS:REG = 300 ms (settling time = 10 frames)
	//300ms delay
...
void OV7670_frame_control(uint8_t size, uint8_t vstart, uint16_t hstart, uint8_t edge_offset, uint8_t pclk_delay) {
  uint8_t value;
  ...
  uint16_t vstop = vstart + 128;
  uint16_t hstop = (hstart + 160) % 304;
...
void OV7670_night(OV7670_night_mode night) {
  ...
  static const uint8_t night_bits[] = {0, 160, 192, 224};
  ...
  com11 = com11 & 0b00011111; // Clear night mode bits
  com11 = com11 | night_bits[night]; // Set night bits for desired mode
